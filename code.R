...
#Defining qdap_clean function where all the preprocessing function are applied on the dataset.
...
#Making a function tm_clean for performing cleaning on the data. 
tm_clean <- function(x){
  x <- replace_abbreviation(x)
  x <- replace_contraction(x)
  x <- replace_number(x)
...
wordcloud(words = names(amzn_p_freq), frequency = amzn_p_freq, max.words = 25, color = "blue")
...
wordcloud(words = names(amzn_c_freq), freq = amzn_c_freq, max.words = 25, color = "red")
...
findAssocs(amzn_p_tdm, "fast paced" , corlimit = 0.2)
...
# Create top15_df
top15_df <- data.frame(x = common_words[1:15,1], y = common_words[1:15,2],
